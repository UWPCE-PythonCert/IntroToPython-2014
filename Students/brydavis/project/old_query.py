...
# json_file = open("new.json", "r").read()
json_file = json.load(open("new.json", "r"))
...
class json_parse (object):
	...
	def get_keys(obj):
		for n,d in enumerate(data):
			if len(store) <= level:
				store.append(set(d.keys()))
			else:
				store[0].union(d.keys())
...
def json_to_sql (data,level=0):
	...
	if isinstance(data, dict):
		...
		for n,k in enumerate(keys):
			# print "k: %s, keys[k]: %s, data[k]: %s" % (k,keys[n],data[k])

			kind = json_to_sql(data[k],next_level)
			if kind:
				# print "\t"*next_level,"key: %s, type(data[k]): %s" % (k,type(data[k])),json_to_sql(data[k],next_level)
		...
		for k,d in enumerate(data):
			...
			if kind:
				# print "\t"*level,k,json_to_sql(d,next_level)
...
for a in data:
	for b in a.keys():
		store.add(b)
		if isinstance(a[b], list): print("dict: %s" % a[b])
