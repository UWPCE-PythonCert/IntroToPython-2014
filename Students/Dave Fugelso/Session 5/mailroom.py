...
def thankYouString (fullname, donations):
    '''
    Send thank you letter using format instead of piecing the string together. Return the string instead of printing.
    '''
    donationString = ''
    if len(donations) > 1:
        for dollars in donations[:len(donations)-2]:
            donationString += str(dollars)+', '
        donationString += 'and ' + str(donations[len(donations)-1])
    else:
        donationString = str(donations[0])
    thankYouString = '\n\n\n\n\t\t\t\t\t{date}\nOur Charity Name\nAddress\nEtc\n\n\nDear {name}:\nThank you for your donation(s) of {donations}. \
We look forward to serving our community blah blah blah.\n\nSincerely, \n\nOur Charity.\n\n\n\n'
    return thankYouString.format(date=datetime.date.today(), name=fullname, donations=donationString)
...
def processDonors ():
    ...
    processing = True
    while processing:
        action = raw_input ("Select 'Send a Thank You(S)', 'Send Thank you to All(A), ('Create a report(C)', or 'Quit(Q)': ")
        if action.upper() == 'S' or action.upper() == 'SEND A THANK YOU':
            thankYouProcessing = True
            while thankYouProcessing:
                name = raw_input ("Find donor (F), add a donor ('A') or ('Add'), or list all donors ('L') or ('List') or 'E' or 'End' to go to main menu: ")
                if name.upper() == 'L' or name.upper() == 'LIST':
                    listDonors()
                elif name.upper() == 'E' or name.upper() == 'END':
                    thankYouProcessing = False
                elif name.upper() == 'F' or name.upper() == 'FIND':
                    donorNames = donors.keys()
                    l = len (donors)
                    print 'Use arrow keys to find donor then select <Enter>'
                    selected = False
                    nameIndex = 0
                    import msvcrt
                    prevch = 0
                    while not selected:
                        if prevch != 224:
                            print '\r                                                                    \r'+ donorNames[nameIndex], '(Enter to select):',
                        ochars = ord(msvcrt.getch())
                        if ochars == 72 and nameIndex > 0:
                            nameIndex -= 1
                        elif ochars == 80 and nameIndex < len (donors)-1:
                            nameIndex += 1
                        elif ochars == 13:
                            thankYou (donorNames[nameIndex], donors[donorNames[nameIndex]])
                            selected = True
            ...
            processing = False
        elif action.upper() == 'A' or action.upper() == 'ALL':
            for name in donors.keys():
                file_out = open(name+'.txt', 'w')
                file_out.write (thankYouString (name, donors[name]))
                file_out.close()
