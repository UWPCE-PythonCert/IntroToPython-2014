...
def trapzz(fun, a, b):
    ...
    Trap0 = 0 # Define the first trapazoid
    TrapN = N[-1] # Define the last trapazoid
    TrapCumulativeArea = 0
    for Trap in N:
        if Trap == Trap0 or Trap == TrapN:
            if Trap == Trap0:
                x = a
            else:
                x = a + Trap
            TrapArea = math.sqrt(x-1)
        else:
            x = a + Trap
            TrapArea = 2*(math.sqrt(x-1))
        # End if
        #print("Trapazoid Area =",TrapArea)
        TrapCumulativeArea = TrapCumulativeArea + TrapArea
        TrapArea = TrapCumulativeArea / 2
        #print("Trapazoida Cumulative Area =",TrapCumulativeArea)
        #print("Trapazoida Area =",TrapArea)
    # End for
    return(TrapArea)
...
# Calculate the area for an equation squt(x-1) Method1
# CurveEqArea1 = trapzz(CurveEq, a, b)
# print("Trapazoidal Calcualtion for Squt(x-1) =", CurveEqArea1)
